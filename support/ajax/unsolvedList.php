...
$where = "WHERE p.solved=0";
if($_SESSION['USERID'] == '9999991' ){
    if(isset($_POST['owner'])){
    $owner= $_POST['owner'];
    if($owner!=0){
    $where = "WHERE p.solved=0 AND p.propietari = $owner";
    }
    }
}else{
...
<ul>
                            ...
                            $CLD_CON->OpenRs("SELECT p.id as IDN , p.booth_id bi,  p.dataTiempo , bt.name as nombre FROM SAT_problems p LEFT JOIN booth_types bt ON bt.char = p.boothType   $where ORDER BY p.dataTiempo DESC");
                            while($CLD_CON->FetchArray()){
								$serialnumber = "";
                                $id = $CLD_CON->GetArrayField("IDN");
                                $data = $CLD_CON->GetArrayField("dataTiempo");
                                $typeName = $CLD_CON->GetArrayField("nombre");
								$photoBooth = $CLD_CON->GetArrayField("bi");															
