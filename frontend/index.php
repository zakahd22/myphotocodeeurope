...
class index_view{
    ...
    private function define_elements(){
        ...
        if (isset($_GET['error'])) {
            $this->error = htmlspecialchars($_REQUEST['error']);
        }

        if (isset($_REQUEST['code'])) {
            $this->input_value = htmlspecialchars($_REQUEST['code']);
            $_SESSION['photoCode'] = htmlspecialchars($_REQUEST['code']);
        } 
        else {
...
//            }
        ...
        if (isset($_REQUEST['token'])) {
            $this->getTokenFromPost(htmlspecialchars($_REQUEST['token']));
...
HTML;
    ...
    private function getTokenFromPost($token){
        $CLD_CON = getNewBdD();
        $sql = "SELECT code FROM `gestor` WHERE token = ?"; 
        $CLD_CON->OpenRs($sql, "s", $token);
