...
$CLD_CON->OpenRs("SELECT b.serialnumber , b.name, ba.when , ba.typeAlert FROM App_boothAlert ba   LEFT JOIN App_booths b ON b.idBooth = ba.idBooth WHERE ba.idBooth IN($IN) AND ba.estat<2 AND ba.typeAlert=54 ORDER BY ba.when DESC");
$html .= "<div class='inContent'>";
$html .= "<h1>CAMERA <span style='background-color:#FF9999;color:#FF0000;'>ERRORS</span></h1>";
$html .= "<div class='noSolvedAlert'>";
if($CLD_CON->GetRsRows()==0){
    $html .= "No Camera errors";
}
while ($CLD_CON->FetchArray()) {
 ...
 $ty = $CLD_CON->GetArrayField("typeAlert");
    if ($ty == 54) {
//         $html .= "<p class='alertsRed'>$date - PhotoBooth $sn-$name  has a camera error</p>";
         $html .= "<p class='alertsRed'>$date - PhotoBooth $sn  has a camera error</p>";
    }
...
$CLD_CON->OpenRs("SELECT b.serialnumber , b.name, ba.when , ba.typeAlert FROM App_boothAlert ba   LEFT JOIN App_booths b ON b.idBooth = ba.idBooth WHERE ba.idBooth IN($IN) AND ba.estat=2 AND ba.typeAlert=54 ORDER BY ba.when DESC");
$html .= "<h1>SOLVED CAMERA ERROR</h1>";
...
while ($CLD_CON->FetchArray()) {
    ...
    $date = $CLD_CON->GetArrayField("when");
     $ty = $CLD_CON->GetArrayField("typeAlert");
    if ($ty == 54) {
//         $html .= "<p >$date - PhotoBooth $sn-$name  had a camera error</p>";
         $html .= "<p >$date - PhotoBooth $sn  had a camera error</p>";
    }
}
$html .= "</div>";
$html .= "</div>";
