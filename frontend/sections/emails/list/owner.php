...
$photos = $baseController->photosModel->getCodePhotosOwner($owner);
$i = 0;
$in = " ";
foreach ($photos as $photo){
    $photos_codes[$i] = $photo['event_id'];
    $in .= "'" . $photo['code'] . "' ,";
    $i++;
}
$in = substr($in, 0, -1);
...
$select_nolimit = "SELECT e.code FROM registre_emails e WHERE e.code in($in) GROUP BY e.email ORDER BY e.email";
$totalrows = count($baseController->registre_emailsModel->getRegistreEmailsOwner($photos_codes));
$regEmails = $baseController->registre_emailsModel->getRegistreEmailsOwner($photos_codes, $LIMIT);
