...
//201306 primer el booth i després les fotos INICI
$sql = "SELECT App_booths.type FROM App_booths WHERE idBooth=$idBooth";
$esOK = $APP_BdD->OpenRs($sql);
if(!$esOK){
//caldria controlar l'error
    echo "$APP_xml<comm_status>Error Database error code: 0000.1 </comm_status></return>";
    return;
}
if($APP_BdD->FetchRs()){
    $lletraPB =  $APP_BdD->GetField(1);
}
else{
    echo "$APP_xml<comm_status>Error Booth not found</comm_status></return>";
    return;
}

//201306 primer el booth i després les fotos FINAL
...
$xml.= "<booth_url>{$url}boothimage/$lletraPB.png</booth_url>";
$ambType = true;
$xml.= "<photos>";
