...
$content .= "Add a comment of incident selected";
$content .= "</div>";
...
$buttons .= "<input type='button' class='popup-confirm' value='Save' onclick='addComentIncident($ID); hidePopupv2();'>";
$buttons .= "<input type='button' class='popup-cancel' value='Cancel'  onclick='hidePopupv2();'>";

$content .= <<<HTML
<script>
    function addComentIncident(id){
        var coment = $("#coment").val();
        if(coment.length > 0 ){
           var ajaxData = {id: id , coment : coment };
                $.ajax({
                    url: 'edit/functions/photobooths/newComentIncident.php',
                    type: 'POST',
