...
if (!empty($e)) {
    ...
    $buttons .= "<input type='button' class='popup-confirm' value='Save' onclick='toFinishProduct($ID); hidePopupv2();'>";
    $buttons .= "<input type='button' class='popup-cancel' value='Cancel' onclick='hidePopupv2();'>";

$content .= <<<HTML
<script>
    function toFinishProduct(id){
        var cQ = $("#cQ").val();
        if(cQ === "0"){
            alert("Please select the Control Quality Person");
        }else{
            var ajaxData = {id:id , cQ:cQ};
             $.ajax({
                    url: 'edit/functions/photobooths/toFinishProduct.php',
                    type: 'POST',
