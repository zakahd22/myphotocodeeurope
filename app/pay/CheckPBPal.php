<?php
require("common.php"); 
$f = fopen('./logPalPB.txt', 'a+');
fwrite ($f, "\r\n BdD: $APP_BdD_error;\r\n");
fwrite ($f, "\r\n okBase: $APP_BdD->okBase;\r\n");
fclose ($f);

$ara = new DateTime("now");
//$f = fopen('./util/logPalPB.txt', 'a+');
...
//SandBox $url_parsed=parse_url('https://www.sandbox.paypal.com/cgi-bin/webscr');
  $url_parsed=parse_url('https://www.paypal.com/cgi-bin/webscr');//entorn real
...
$res = trim ($res);//20140314, veure CheckOrderPal
...
//SandBox if (strcmp ($res, "VERIFIED") == 0  || $url_parsed[host] == 'www.sandbox.paypal.com' ) {
if (strcmp ($res, "VERIFIED") == 0  ) {//entorn real
...
if(($payment_status == "Completed") || ($payment_status == "Pending")){//  tot pagat o pending 
    ...
    if($payment_status == "Completed"){
       ...
       $completed = "Completed";
        ...
        $fpagstatus = 5;
       $infoPending = "<h2>This payment is <b>pending</b> and is necessary to go into the account and accept it.</h2>";
       $completed = "Pending";
    }
//20131016 FINAL    
...
//no cal        include("../common/APP_BdD.php");
   //SELECT `idOrder`, `idOwner`, `when`, `shipping`, `total`, `idAdress`, `text`, `address`, `city`, `state`, `code`, `country`,`email`, `fpag`, `fpagcontrol`, `fpagn`, `fpagstatus` FROM `App_orders` WHERE 1
    ...
    $idPB = "";
    $payPalVendor = "";
    $sql ="SELECT idPB, payPalVendor  FROM `App_PBorders` WHERE  idOrder=$idOrder AND `fpagn`=$fpagn;";
...
fclose ($f);
    ...
    if(!$esOK){
        ...
        if(!$esOK){
            ...
            $idPB = $APP_BdD->GetField(1);
            $payPalVendor = $APP_BdD->GetField(2);
            $APP_BdD->CloseRs();
        }
    }
 ...
 //més control      $sql = "UPDATE `App_orders` SET `fpagstatus`=2 WHERE idOrder=$idOrder AND ntpv=$ntpv AND controltpv='$item_number';";
        $sql = "UPDATE `App_PBorders` SET `fpagstatus`=$fpagstatus WHERE idOrder=$idOrder AND `fpagn`=$fpagn;";
        $APP_BdD->Execute($sql);
    $my_string.=  "sql = $sql\r\n";
    
    //es pot enviar un email al client: App_PBorders

}
// un email
$mail_email = "victor.carretero@treemes.com";//email a sales@dc-image.com
$mail_nom = "myphotocode.com";
$mail_subject = "PayPal Payment $completed from a PhotoBooth";
$mail_cont = "<h1>An order from a PhotoBooth has been payed with PayPal</h1>$infoPending<table>";
...
$mail_cont.= "<tr><td>Vendor code:</td><td><b>$payPalVendor</b></td></tr>";
...
include("../common/APP_mail.php");
    ...
    if((isset ($_POST['item_number']))){
        ...
        $sql = "UPDATE `App_PBorders` SET `fpagstatus`=3 WHERE idOrder=$idOrder;";
        $APP_BdD->Execute($sql);
    }
    $my_string.=  "sql = $sql\r\n";
...
$mail_email = "victor.carretero@treemes.com";//email a sales@dc-image.com
$mail_nom = "myphotocode.com";
$mail_subject = "PayPal Payment Not Completed from a PhotoBooth";
