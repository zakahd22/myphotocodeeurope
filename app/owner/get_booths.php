...
$sql = "SELECT App_booths.idBooth, booth_types.name, App_booths.name, myBoothDongle .rand_string, estat, App_booths.latitude, App_booths.longitude
    FROM (App_booths LEFT JOIN booth_types ON App_booths.type = booth_types.`char`)
...
//    $xml.= "<status>$tmp</status>";
    ...
    $xml.= "<location>";
    $tmp = $APP_BdD->GetField(6); if($tmp) $tmp/=1000000;//$tmp = sprintf ("%f", $tmp/1000000);
    $xml.= "<latitude>$tmp</latitude>";
    $tmp = $APP_BdD->GetField(7); if($tmp) $tmp/=1000000;//
