...
//20120913 no el volen    $xml.= "<booth_status>{$APP_estatsBooth[$tmp]}</booth_status>";
    ...
    if($tmp){//l'alerta offline està definida
        ...
        $tmp = $APP_BdD->GetField(6);
         if($tmp > 12){$tmp-=12;  $pmS="PM";} elseif($tmp == 12) {$pmS="PM";} else {$pmS="AM"; if($tmp == 0){$tmp=12;}}//20130604 
        $textAlertOffline.= sprintf("%02d.",$tmp);
//20130604        $tmp = $APP_BdD->GetField(7); $textAlertOffline.= sprintf("%02d to ",$tmp);
        $tmp = $APP_BdD->GetField(7); $textAlertOffline.= sprintf("%02d",$tmp);//20130604 
        $textAlertOffline.=  " $pmS to ";//20130604 
        $tmp = $APP_BdD->GetField(8); 
         if($tmp > 12){$tmp-=12;  $pmS="PM";} elseif($tmp == 12) {$pmS="PM";} else {$pmS="AM"; if($tmp == 0){$tmp=12;}}//20130604 
        $textAlertOffline.= sprintf("%02d.",$tmp);
        $tmp = $APP_BdD->GetField(9); $textAlertOffline.= sprintf("%02d",$tmp);
        $textAlertOffline.=  " $pmS";//20130604 
    }
    else{
...
//20130521 INICI
//20130521 $sql = "SELECT `App_alerts`.`typeAlert`,`label`,`value`,`App_alerts`.`values` FROM `App_alerts`
//20130521    LEFT JOIN `App_boothAlertDef` ON `App_alerts`.`typeAlert` = `App_boothAlertDef`.`typeAlert`
//20130521    WHERE `idBooth`=$idBooth; ";

$myAlerts = "(SELECT `typeAlert`, `value` FROM `App_boothAlertDef` WHERE `idBooth`=$idBooth) AS myAlerts";

$sql = "SELECT `App_alerts`.`typeAlert`,`label`,`value`,`App_alerts`.`values` FROM `App_alerts`
    LEFT JOIN $myAlerts ON `App_alerts`.`typeAlert` = myAlerts.`typeAlert` 
    WHERE `App_alerts`.`typeAlert` BETWEEN 11 AND 49;";
...
//20130521 INICI
//20130521$sql = "SELECT `App_config`.`typeConfig`,`label`,`value`,`App_config`.`values` FROM `App_config`
//20130521    LEFT JOIN `App_boothConfigDef` ON `App_config`.`typeConfig` = `App_boothConfigDef`.`typeConfig`
//20130521    WHERE `idBooth`=$idBooth; ";

$myConfigs = "(SELECT `typeConfig`, `value` FROM `App_boothConfigDef` WHERE `idBooth`=$idBooth) AS myConfigs";

$sql = "SELECT `App_config`.`typeConfig`,`label`,`value`,`App_config`.`values` FROM `App_config`
    LEFT JOIN $myConfigs ON `App_config`.`typeConfig` = myConfigs.`typeConfig`; ";
