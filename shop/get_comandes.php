...
$CLD_CON->OpenRs("SELECT * FROM SHP_Comandes ORDER BY fecha DESC");
while ($CLD_CON->FetchArray()) {
    ...
    if($printed==1){
    echo "<img src='images/print.png' style='width:24px;height:24px;cursor:pointer;background-color:green;box-shadow: 1px 1px 10px green;' onclick='print0(\"$id_c\")'>";    
    }else{
    ...
    echo "<div style='width:90%;display:none;background-color:white;color:black;margin-left: 5%;' id='con_add$id_c' class='order_info'>";
    echo $address;
    echo $contact;
    echo "</div>";
    $CLD_CON2->OpenRs("SELECT * FROM SHP_Comandes_Products WHERE comanda=$id_c");
    echo "<table style='width:90%;margin-left:5%;display:none;background-color:white;' border='1' id='t$id_c' class='table_info'>";
...
/*echo "<h2 class='titles'>Printed orders :</h2>";
$CLD_CON->OpenRs("SELECT * FROM SHP_Comandes WHERE (estat = 4 OR estat= 5) AND printed=1 LIMIT 25");
while ($CLD_CON->FetchArray()) {
    $tt = 0;
    $id_c = $CLD_CON->GetArrayField("id");
    $fecha = $CLD_CON->GetArrayField("fecha");
    ...
    echo "<div class='comanda' id='c$id_c' style='background-color:#808000;color:white;'>";
    echo "<p>Order $id_c <span style='display:inline;float:right;'>$f_date";
    echo "<img src='images/eye.png' style='width:24px;height:24px;cursor:pointer;' onclick='open_Close(\"$id_c\")'>";
    echo "<img src='images/print.png' style='width:24px;height:24px;cursor:pointer;' onclick='print0(\"$id_c\")'>";
    echo "</span></p>";
    echo "<div style='background-color:white;width:100%;height:auto%;'>";
    echo "<div style='width:90%;display:none;background-color:white;color:black;' id='con_add$id_c' class='order_info'>";
    echo $address;
    echo $contact;
    echo "</div>";
    $CLD_CON2->OpenRs("SELECT * FROM SHP_Comandes_Products WHERE comanda=$id_c");
    ...
    $html2 .= "<tr style='padding:20px;margin:5px;'><td colspan='3' style='padding:20px;margin:5px;'></td><td style='padding:20px;margin:5px;'>TOTAL </td><td style='padding:20px;margin:5px;'>" . ($tt + $imp) . "$currency_symbol</td></tr>";
    $html2 .= "</table>";
if (!file_exists("comandes/$shop_c/$id_c/$id_c.pdf")) {
        $html = "<html><head></head><body>";
        $html .= "<p>Order $id_c ,  $f_date</p>";
        $html .= "<div style='width:100%;'>";
        $html .= $address;
        ...
        $html2pdf = new HTML2PDF('P', 'A4', 'fr');
        $html2pdf->writeHTML($html);
        mkdir("./comandes/$shop_c/$id_c");
        $output_file = "./comandes/$shop_c/$id_c/$id_c.pdf";
        $html2pdf->Output($output_file, 'F');         
  }
    echo "<iframe src='./comandes/$shop_c/$id_c.pdf' width='0px' height='0px' id='iframe$id_c' style='border:0px solid transparent;'></iframe>";
    echo "</div>";
       echo "</div>";
}*/
?>