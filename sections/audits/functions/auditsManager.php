    ...
    public function indexAction() {
        ...
        switch ($this->action) {
            ...
            case 'getOwnerPBs':
                ...
                list($success, $message) = $this->getOwnerPBs($this->idUser, $this->serialNumber);
                if ($success) {
                    $result = array();
                    $i = 0;
...
HTML;
    ...
    private function getAuditsView() {
        ...
        if ($this->individual !== FALSE) {
            if ($this->idPb == FALSE) {
                $Pb = $this->RepdcModel->getOwnerPb($this->idUser);
            ...
            $html .= <<<HTML
               <div id="audits_wrap">
                <div id="primer">
                    ...
                    <div id="pbList">
                        <div id="pbList_title">
                            <h5>PhotoBooth List</h5>
                        </div>
                        <div id="contentFilterPb">
                            <input class='serialNumberInput' type="text" name="serialNumber" placeholder='Insert the Photobooth S/N' value="" autocomplete="off" autofocus>
                            <button type="button" class="popup-confirm selectedBtn" id="allPbs">All Pbs</button> 
                        </div>
                        <div id="pbListTable">
                            {$PbList}
                        </div>
    ...
    private function getPbInfo() {
        ...
        if ($this->individual !== FALSE) { 
            ...
            if ($pbInfo[0] !== FALSE) {
                ...
                $lastConnLocal = $RepdcManager->getPbLastConnectionLocal($this->idPb);
                ...
                $companyName = "";
                $rentals = $this->rentalsModel->getRentalsById($this->idUser);
                if ($rentals) {
                    $companyName = stripslashes($rentals[0]["name"]);
                }
                $owner_row = '<div id="idOwner" class="hidden">'.$this->idUser.'</div>';
                if($this->userType==1){
                    $owner_row = '<div class="label">Owner ID:</div><div id="idOwner" class="value">'.$this->idOwner.'</div>';
                }
...
HTML;
            ...
            $info_title = "GLOBAL ";
            $info_text = "<div id='idPb'></div><div id='idOwner' class='hidden'>$this->idOwner</div>";
        }
...
HTML;
    ...
    private function getPbList() {
        ...
        foreach ($PbList as $pb) {
            ...
            $html .= <<<HTML
                <div class='row {$selected}' idPb='{$pb['idBooth']}' serialNumber='{$pb['serialnumber']}' individual='true'>
                    <p class='text'>
...
//                utils::log($array_info, 'logAleix');
                    ...
                    $array_infoVersion = $RepdcManager->getSummaryByDayInfo($idPb, $intervalStart, $intervalEnd, 4); //consultem nomes versio

                //$minPBversion = $this->checkPBVersion($array_info);
                $minPBversion = $this->checkPBVersion($array_infoVersion); //20220428 en casos en que nomes hi ha 50, 60 i cap partida no es mostren les hores d'activitat ni l'overpayment perquè array_info només conté partides.
//                $minPBversion = 21;
//                utils::log("minVersion = {$minPBversion}", 'logAleix');
                ...
                if($minPBversion >= 21){
                    ...
                    $hoursOperation_label = "<td class='rotate borderLeft info' style='height: 127px;' {$title}><div><span>Hours Operation*</span></div></td>";
                    $overpayment_label = "<td class='rotate borderRight info' style='padding-bottom: 0px;' {$title}><div><span>Overpayment*</span></div></td>";
                }
                
                $stock_label = "";
                $collection_label = "";
                if($idPb){
                    $stock_label = "<td class='rotate borderRight'><div><span>Stock</span></div></td>";
                    $collection_label = "<td class='no-color td-small'></td><td class='rotate borderRight'><div><span>Collection</span></div></td>";
                }
                $twilio_label ="";
                if($this->userType==1){
                   $twilio_label ='<td class="rotate"><div><span>Whats Qty</span></div></td><td class="rotate borderRight"><div><span>SMS Qty</span></div></td><td class="rotate"><div><span>Whats Cost</span></div></td><td class="rotate"><div><span>SMS Cost</span></div></td><td class="rotate borderRight totalization"><div><span>Total Cost</span></div></td>';      
                }
                $html .= $error_1;
                $html .= $error_2;
                //Completed
                $html .= <<<HTML
                        <table class="reportTable">
                            <tr class="labelsTable {$translateClass}">
                                ...
                                <td class="rotate borderLeft"><div><span>Plays</span></div></td>
                                <td class="rotate"><div><span>FreePlays</span></div></td>
                                <td class="rotate"><div><span>Upsells</span></div></td>
                                <td class="rotate"><div><span>Passport</span></div></td>
                                <td class="rotate"><div><span>SmartPrint</span></div></td>
                                <td class="rotate borderRight totalization"><div><span>Prints</span></div></td>
                                <td class="rotate"><div><span>Cash</span></div></td>
                                <td class="rotate"><div><span>CreditCard</span></div></td>
                                <td class="rotate"><div><span>Net</span></div></td>
                                <td class="rotate borderRight totalization"><div><span>Money</span></div></td>
                                {$overpayment_label}
                                {$stock_label}
                                {$collection_label}
                                {$twilio_label}
...
//                            $ultDia = str_replace("/", "-", $ultDia);
                       $html.= <<<HTML
                            ...
                            <button id="csvbutton" type="button" onclick="CSV('{$auditInfo['number']}','$primDia','$ultDia')" text="" >CSV</button>
HTML;
...
HTML;
    ...
    private function getAllAudit($n = 0, $auditInfo = FALSE) {
        $AuditInfo = "";
        $Audit = "";
        $AuditList = "";
        ...
        if ($this->startDate !== FALSE) {
            ...
            $dates = $intervalStart.'-'.$intervalEnd;
            $inici = $auditInfo['startDate'];
        } else {
...
return $html;
    ...
    private function getSumaryWeeklyReport($startDateTime, $endDateTime, $array_info, $idPb, $minPBversion) {
        ...
        $lastInfo_ = $lastInfo->format('Y-m-d');
...
//            utils::log("Hours Operation = {$hoursOperation}", "logAleix");
            ...
            $collection_unknown = "";
            if($idPb){
                //Busquem l'últim stock conegut
//               $stock_empty .= "<td class='borderRight'>-</td>";
                 $stockLast =  $this->RepdcModel->getLastStockByPb($idPb, $startDateTime);
                 $stock_empty .= "<td class='borderRight'>".$stockLast."</td>";
            ...
            if($this->userType==1){
                $twilio_empty = "<td>0</td><td class='borderRight'>0</td><td>0</td><td>0</td><td class='borderRight totalization'>0</td>";
                $twilio_unknown = "<td>-</td><td class='borderRight'>-</td><td>-</td><td>-</td><td class='borderRight totalization'>-</td>";
             }

            $lastStyle = "";
            if($lastInfo_ == $day_){
                $lastStyle = "lastDataRow";
            } 
...
//                    $freePlays = $this->calcPlaysFP($array_info[$day_]['freePlays'], intval($pasport), intval($SmartPrint));
                    ...
                    utils::log('auditsManager AUDIT DAY', 'logAleix');
                   ...
                   if($overpayment<0){
                    ...
                    if(is_null($array_info[$day_]['stock'])){

                        
                        //Busquem l'últim stock conegut
                       $array_info[$day_]['stock'] =  $this->RepdcModel->getLastStockByPb($idPb, $startDateTime);
                    ...
                    $qtyWhats   = ($array_info[$day_]['qtyWhats']) ? $array_info[$day_]['qtyWhats'] : "0"; 
                    $qtySMS     = ($array_info[$day_]['qtySMS']) ? $array_info[$day_]['qtySMS'] : "0"; 
...
//                                utils::log('TRACE 0', 'logAleix');
                    ...
                    $twilio_row = "";
                    if($this->userType==1){
                        $twilio_row = "<td>{$qtyWhats}</td><td class='borderRight'>{$qtySMS}</td><td>{$costWhats}</td><td>{$costSMS}</td><td class='borderRight totalization'>{$costTwilio}</td>";
                    }
...
//                    }
        ...
        if($idPb){
            ...
            $TotalCollection_rows .= "<td class='no-color td-small'></td><td class='no-color_ borderRight'></td>";
        }
        
        if($this->userType==1){
            $TotalTwilio_rows = "<td class='totalization'>{$totalQtyWhats}</td><td class='totalization borderRight'>{$totalQtySMS}</td><td class='totalization'>{$totalCostWhats}</td><td class='totalization'>{$totalCostSMS}</td><td class='borderRight totalization'>{$totalCostTwilio}</td>";
...
//            print_r($this->hoursOperationArray);exit;
...
//                        $freePlays = $this->calcPlaysFP($array_info[$day_]['freePlays'], intval($pasport), intval($SmartPrint));
                        ...
                        $creditCard  = ($array_info[$day_]['i4']) ? $array_info[$day_]['i4'] : "0";
                        utils::log('creditCard inicial:'.$creditCard, 'logAleix');
                        $creditCard_60     = ($array_info[$day_]['i4_60']) ? $array_info[$day_]['i4_60'] : "0";
                        $creditCard        = intval($creditCard) + intval($creditCard_60);
                        $net         = ($array_info[$day_]['i5']) ? $array_info[$day_]['i5'] : "0";
                        $money       = ($array_info[$day_]['money_']) ? $array_info[$day_]['money_'] : "0";
...
//    private function calcPlaysFP($freePlays, $pasport, $SmartPrint) {
////        if($freePlays !== FALSE){
//            $freePlays = $freePlays - $pasport - $SmartPrint;
////        }
//        return $freePlays;
//    } 
