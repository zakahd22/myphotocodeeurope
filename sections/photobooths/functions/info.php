...
if ($CLD_CON->FetchArray()) {
    ...
    $dateOwner = $CLD_CON->OpenRs("CLD_date_tOwner");
...
echo "<img src='images/web/pb/$typeID.png' style='width:100%;'>";
echo "</div>";
...
if($_SESSION['USERTYPE']<4){
    ...
    if($status==3){
        ...
        if(!empty($dateOwner)){
            echo "<p> Date sold to Owner : $dateOwner</p>";
        }else{
            echo "<p> Date sold to Owner : Undefined</p>";
        }
    }   
    if($status==4){
...
echo "<p>$location <input type='button' class='editButton' onClick='edit(6 , $ID);'></p>";
echo "<h3>GPS Position <input type='button' class='editButton' onClick='edit(7 , $ID);'></h3>";
...
if (empty($paypal)) {
    echo "<h1>PAYPAL IS <span style='color:red;'>OFF</span> <input type='button' class='editButton'  onClick='edit(8 , $ID);'></h1>";
} else {
    echo "<h1>PAYPAL IS <span style='color:green;'>ON</span> <input type='button' class='editButton'  onClick='edit(8 , $ID);'></h1>";
}
...
echo "<div class='box'>";
if ($value == "NO") {
    echo "<h1>EMAIL REPORTS ARE <span style='color:red;'>OFF</span> <input type='button' class='editButton' onClick='edit(9 , $ID);'></h1>";
    echo "<p> The email reports option is OFF for this PhotoBooth. </p>";
}
if ($value == "YES") {
    echo "<h1>EMAIL REPORTS ARE <span style='color:green;'>ON</span> <input type='button' class='editButton' onClick='edit(9 , $ID);'></h1>";
