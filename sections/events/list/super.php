...
if (isset($_POST['fil'])) {
    ...
    $events = $baseController->eventsModel->getAllFromEventsListIn($in, $tt, $id);
    $totalrows = count($events);
} 
else {
    $events = $baseController->eventsModel->getAllFromEventsList(false, $LIMIT);
    $totalrows = count($baseController->eventsModel->getAllFromEventsList(false));
}
