...
//        var_dump($igUser);exit;
      ...
      $getLastDownloadedPhotos =  $igUser->getLastDownloadedPhotos();
        ...
        $urlsArray = getNextPhotos($word, $filename, $token, $type, $getLastDownloadedPhotos , $photos_number, $photos_number, $other); 
// print "++++++urlsarray<pre>";
//         print_r($urlsArray);
        $urls = $urlsArray['urls'];
        ...
        if(isset($urls) && $urls != "" ){
            ...
            if($urlsDownloaded==''){ //si està en blanc no tenim res descarregat anteriorment i la crida ha fallat (tampoc ha descarregat res nou)
                $result = "RT#$urlsDownloaded";
            }else{
                $result = "OK#$urlsDownloaded";
            }
//            print "urls".$urlsDownloaded."down";
            $result = "OK#$urlsDownloaded";
...
//       print "word".$word."typerequest".$typeRequest."pais".$pais."numfrom".$numFrom."numto".$numTo."other".$other."debugeloi <pre>";
...
//print "arrayDownloadedPhotos<pre>";
//   print_r($arrayDownloadedPhotos);
    ...
    foreach($arrayDownloadedPhotos as $imageDownloaded){
        $urlsDownloaded .= PHOTO_DOMAIN . "/API/instagram/smartprint/photo.php?idphoto=".$imageDownloaded['id']."&w=".$imageDownloaded['word']."&ext=jpg&tkn={$token}|jpg|".$imageDownloaded['numLikes']."|".$imageDownloaded['id']."|".urlencode($imageDownloaded['photoText'])."|";
    }
...
function updateSuggestion($word, $print, $type, $pais, $numFollowers) {
    global $igRepository;

    return $igRepository->updateSuggestion($word, $print, $type, $pais, $numFollowers);
}
