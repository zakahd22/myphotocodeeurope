...
/*  ****************************************************************
					...
					https://www.mjdigital.co.uk/blog/php-upload-and-resize-image-class/
                    
                    Version 1.1         (15 Nov 2011)
                    
                    Copyright MJDIGITAL Â©2009-2011
...
*/
    ...
    public function doDie($msg,$file="",$line="") {                     // doDie - calls a die function with custom error
        ...
        die(	'<h4>Error:</h4>'.$msg.'<br/><br/>'.
                ...
                '</body></html>');
			//echo "<script type='text/javascript'>alert('Error: ".$msg."\nFile: ".$file." - on line: ".$line."');</script>";
        } else {
        die($msg);
		//echo "<script type='text/javascript'>alert('".strip_tags($msg)."');</script>";
        }
    }
} // END OF CLASS
...
{
	$myImage = new _image;
	$myImage->uploadTo = 'usbs/2012052212/';
	...
	if($res)
	{
	    ...
	    $i = $myImage->resize();
	    echo $i."<br /><img src='".$i."' style='border:1px solid #000;' />";
	}
