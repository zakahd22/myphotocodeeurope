 ...
 * 
 * 
  SELECT ab.owner, p.pbs_id FROM `photos` p 
 LEFT JOIN App_booths ab ON ab.idBooth=p.pbs_id  
  WHERE p.code = 'F6S42VK2H4' 
 * 
 * 
 SELECT *  FROM `gestor` g
 -- LEFT JOIN photos p ON p.code = g.code 
 LEFT JOIN App_booths ab ON ab.idBooth=p.pbs_id 
 
 WHERE 
 --g.state=6
 --AND (g.method=1 OR g.method=3)
  -- AND 
 g.code = 'HKPXNX5YPW' 
 AND g.timestamp BETWEEN '2019-11-01 00:00:00.000000' AND '2019-12-01 00:00:00.000000' ORDER BY `last` DESC
  
 * 
 * 
 * 'F6S42VK2H4' codi 172.16.3.22
 * 
 * 'FFDZCAGB4U' codi MyPC.com ionos
 * 
 */


//TODO: canviar variables fixes per les de taula gestor quan s'enviï
$codiFoto = "FFDZCAGB4U";
$contacte = "+999 652952049";
...
//print_r($arrayCodesISO);
$ISOCountry = $arrayCodesISO[$prefix[0]]['ISO'];
$costWhatsapp = $arrayCodesISO[$prefix[0]]['costWhatsapp'];
...
$arrayCost = json_decode($response);
...
if (!curl_errno($curl)) {
        ...
        $arrayCost = json_decode($response);
        //print_r($arrayCost);
        //TODO: Creem registre del que costa aquest SMS canviem UPDATE per INSERT a taula gestor
        ...
        * //TODO: fer alguna cosa si va malament ;)
...
print "ISO: ".$ISOCountry;
print "<pre>";
print_r($arrayCost);
print "price unit: ".$arrayCost->price_unit;
print "current price: ".$arrayCost->outbound_sms_prices[0]->prices[0]->current_price;   
print "costWhatsapp: ".$costWhatsapp;



    $sql = "SELECT ab.owner, p.pbs_id FROM `photos` p 
 LEFT JOIN App_booths ab ON ab.idBooth=p.pbs_id  
...
//    echo $sql;
    ...
    while ($CLD_CON->FetchArray()) {
        ...
        $idBooth = $CLD_CON->GetArrayField("pbs_id");
    }  
print "owner: ".$owner;
print "idBooth: ".$idBooth;    
//    
//    
//    if(!isset($code_exist) && !isset($contact_exist)){
//       
//        if($versioPB != 'web'){
//                $CLD_CON->Execute("INSERT INTO gestor(`code`, `method`, `contact`, `timestamp`, `state`, `versioPB`, `token`) VALUES ('$codiFoto', '$metode', '$contacte', '$now', 0, '$versioPB', '$token')");
//        }
//
//        else{ 
//             utils::log("INSERT INTO gestor(`code`, `method`, `contact`, `timestamp`, `state`, `versioPB`, `token`) VALUES ('$codiFoto', '$metode', '$contacte', '$now', 1, '$versioPB', '$token')", "logGestor");
//            $CLD_CON->Execute("INSERT INTO gestor(`code`, `method`, `contact`, `timestamp`, `state`, `versioPB`, `token`) VALUES ('$codiFoto', '$metode', '$contacte', '$now', 1, '$versioPB', '$token')");    
//           
//
//        }
//        
