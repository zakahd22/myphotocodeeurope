...
$content .= "</div>";
$content .= "<select id='dEventSel' class='popup-input-large' style='width: 200px; margin-bottom: 10px;'>";
$content .= "<option value=0>None</option>";
$CLD_CON->OpenRs("SELECT id , title FROM events WHERE rental_id = {$ID}");
while ($CLD_CON->FetchArray()) {
    $event_id = $CLD_CON->GetArrayField("id");
    $event_title = $CLD_CON->GetArrayField("title");
    $event_title = str_replace("_", " ", $event_title);
    $content .= "<option value='$event_id'>$event_title</option>";  
}
$content .= "</select>";
...
$content .= "</div>";
$content .= "<select id='dCollagesSelTem' class='popup-input-large' style='width: 200px;' event='$ID'>";
$content .= "<option value=0>None</option>";
